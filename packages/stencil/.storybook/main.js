...
module.exports = {
  ...
  async webpackFinal(config) {
    ...
    fs.readdirSync(path.join(__dirname, OUTPUT_DIR, PROJECT_NAME)).map(file => {
      jsFilePath = path.join(__dirname, OUTPUT_DIR, `${PROJECT_NAME}/${file}`);
      try {
        if (fs.existsSync(jsFilePath) && /\.entry\./.test(jsFilePath)) {
